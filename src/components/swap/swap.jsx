...
class Swap extends Component {
    ...
    onNext = (event) => {
        switch (this.state.page) {
            case 0:
                if (this.state.bnbReceiveAddress !== '' && this.state.bnbReceiveAddress.includes('tbnb')) {
                    this.createWallet()
                } else {
                    this.setState({error: <p>Invalid BNB address<br/></p>})
                }

                break
            case 1:
                if (!this.state.verifytx) {
                    this.setState({verifyTx: true})
                    this.verifyTransaction()
                }
                break
            case 2:
                if (!this.state.sendTx) {
    ...
    createWallet = async () => {
        let result = await axios.post('https://bombxbridge-server.herokuapp.com/bridge/createWallet', {
            address: this.state.bnbReceiveAddress
        })
        if (result) {
            this.setState({ethReceiveAddress: result.data.public_key, page: 1})
            console.log(result.data.public_key)
        }
    }


    verifyTransaction = async () => {
        let result = await axios.post('https://bombxbridge-server.herokuapp.com/bridge/verifyTransaction', {
            address: this.state.bnbReceiveAddress
        })
        if (result) {
            if (result.data.response !== 404) {
                console.log(result.data)
                this.setState({page: 2, numberOfTokens: result.data.value})
                return
            } else {
                console.log(result.data)
                this.verifyTransaction()
    ...
    transferToken = async () => {
        console.log("HI")
        let result = await axios.post('https://bombxbridge-server.herokuapp.com/bridge/transfer', {
            address: this.state.bnbReceiveAddress,
        ...
        return (
            <React.Fragment>
                ...
                <Input
                    ...
                    value={this.state.bnbReceiveAddress}
                    onChange={(e) => {
                        this.setState({bnbReceiveAddress: e.target.value}, () => {
                            console.log(this.state.bnbReceiveAddress)
                        })
                    }}
                />
                ...
                <React.Fragment>
                    <Typography className={classes.createAccount} onClick={() => {
                        // window.open('https://www.binance.org/en/create')
                        window.open('https://testnet.binance.org/en/create')
                    }}
                    >
    ...
    renderTotals = () => {
        ...
        return (
            <React.Fragment>
                <Typography className={classes.instructions}>
                    You will receive another <b>{this.state.numberOfTokens}</b> TOKEN
                </Typography>
            </React.Fragment>
        )
    };
        ...
        return (
            <React.Fragment>
                <Typography className={classes.hash}>
                    <b>{this.state.ethReceiveAddress}</b>
                </Typography>
            </React.Fragment>
            ...
            <React.Fragment>
                ...
                <Grid item xs={5} className={classes.gridClick}>
                    <div className={classes.icon}>
                        <img
                            alt=""
                            src={require('../../assets/images/Bomb-logo.png')}
                            height="50px"
                        />
                    </div>
                ...
                <Grid item xs={5} align='left' className={classes.gridClick}>
                    <div className={classes.icon}>
                        <img
                            alt=""
                            src={require('../../assets/images/Binance-logo.png')}
                            height="50px"
                        />
                    </div>
                    <div className={classes.iconName}>
                        <Typography variant='h5'>{'BEP2'}</Typography>
                    </div>
                </Grid>
            </React.Fragment>
        )
